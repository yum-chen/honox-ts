...
export function Root(props: RootProps) {
  ...
  const state = value === null ? "indeterminate" : value >= max ? "complete" : "loading";
...
export function Range(props: PropsWithChildren<{ class?: string; style?: any }>) {
  const { children, class: classProp, style, ...rest } = props;
  const { styles, value, min, max, orientation } = useProgressContext();
  const state = value === null ? "indeterminate" : value >= max ? "complete" : "loading";

  const percent = value === null ? undefined : Math.min(100, Math.max(0, ((value - min) / (max - min)) * 100));

  const rangeStyle = {
    ...style,
    [orientation === "horizontal" ? "width" : "height"]: percent !== undefined ? `${percent}%` : undefined,
  };
...
export function ValueText(props: PropsWithChildren<{ class?: string }>) {
  ...
  const percent = value === null ? 0 : Math.round(((value - min) / (max - min)) * 100);
...
export interface ViewProps extends PropsWithChildren<{ state: "indeterminate" | "loading" | "complete"; class?: string }> {}

export function View(props: ViewProps) {
  const { children, state: targetState, class: classProp, ...rest } = props;
  const { styles, value, max } = useProgressContext();
  const currentState = value === null ? "indeterminate" : value >= max ? "complete" : "loading";
...
export function CircleTrack(props: PropsWithChildren<{ class?: string; style?: any }>) {
...
export function CircleRange(props: PropsWithChildren<{ class?: string; style?: any }>) {
  const { class: classProp, style, ...rest } = props;
  const { styles, value, min, max } = useProgressContext();
  const state = value === null ? "indeterminate" : value >= max ? "complete" : "loading";

  const percent = value === null ? 0 : Math.min(100, Math.max(0, ((value - min) / (max - min)) * 100));
