...
import { createContext, useContext, useId, useRef, useEffect, useState } from "hono/jsx";
...
export function ResizeTrigger(props: ResizeTriggerProps) {
  ...
  return (
    <button
      ...
      {...rest}
    >
      {children || <div data-part="resize-trigger-indicator" class={context.styles.resizeTriggerIndicator} />}
    </button>
  );
}
...
export function InteractiveSplitter(props: InteractiveSplitterProps) {
  ...
  const initialSizes = sizeProp || defaultSize || panels.map(p => ({ id: p.id, size: 100 / panels.length }));
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
        ...
        const totalSize = orientation === "horizontal" ? rootRect.width : rootRect.height;
    ...
    const handleTriggerDown = (e: MouseEvent | TouchEvent) => {
        const trigger = (e.target as HTMLElement).closest('[data-part="resize-trigger"]');
        if (!trigger) return;
    ...
    root.addEventListener("touchstart", handleTriggerDown as any, { passive: false });
