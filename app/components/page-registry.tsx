...
import { propsOf, type ComponentBlock } from "./block-types";
...
// so `type`/`children` can never reach the DOM.
const registry: Record<string, BlockRenderer> = {
  stack: (b) => {
    const { children } = b;
    return (
      <Stack {...propsOf(b)}>{renderChildren(children as ComponentBlock[])}</Stack>
    );
  },
  ...
  group: (b) => {
    const { attached, grow, orientation, children, ...rest } = propsOf(b);
    return (
      <Group
        attached={attached}
        grow={grow}
  ...
  hoverCard: (b) => {
    const { triggerText, showArrow, title, description, children, ...rest } =
      propsOf(b);
    const trigger = triggerText ? (
      <Button variant="outline">{triggerText}</Button>
    ) : undefined;
  ...
  menu: (b) => {
    ...
    return (
      <Menu interactive trigger={trigger} items={items || []} {...rest} />
    );
  },
  ...
  skeleton: (b) => {
    const { variant, noOfLines, loaded, children, ...rest } = propsOf(b);
    if (variant === "text") {
      return (
...
// raw CMS payload as JSON to end users in production; now it always renders a
...
export function renderUnknown(block: ComponentBlock): JSX.Element {
