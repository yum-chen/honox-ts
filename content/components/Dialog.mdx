...
**Interactive-First — auto-interactive by default.** A `Dialog` has no meaningful static fallback — opening, focus trapping, and ESC handling all require client-side JavaScript — so it hydrates as an island by default. Pass `interactive={false}` to render a static, inert dialog shell that ships no client JS.
