...
**Interactive-First — interactive by default.** A `DatePicker` hydrates as an island unless explicitly opted out with `interactive={false}`, in which case it renders as static HTML with no client JS.
