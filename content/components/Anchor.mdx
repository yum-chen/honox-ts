...
```json
{
  ...
  "variant": "underline",
  "colorPalette": "blue",
  "children": [{ "blockType": "text", "content": "View Pricing" }]
}
```

The link's content comes entirely from `children` — a list of nested blocks,
same convention as every other container block (Stack, Card, ...). A plain
label is a single `text` leaf; add an `icon` leaf instead (or alongside) for
a brand-mark/icon-only link, e.g. a GitHub link in a header. Every other
field passes straight through to `Anchor`.
