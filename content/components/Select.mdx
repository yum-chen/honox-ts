...
**Interactive-First — auto-interactive.** Opening the dropdown and selecting an option require client JS, and there is no static fallback (the native `<select>` is visually hidden and exists only for form submission), so `Select` hydrates by default. Pass `interactive={false}` to force a purely static render.
