...
Pass `hoverActions` to render extra controls (e.g. a "View Details" button) in a trailing column that stays hidden until its row is hovered or one of its controls receives keyboard focus.
...
| **Hover Actions** (`hoverActions`) | code-only | - | `(row, rowIndex) => JSX.Element`. Renders into a trailing column cell that's hidden until the row is hovered (or a rendered control gains keyboard focus). Not exposed as a CMS field. |
